...
“取回”会请求已注册的 TUI 完成当前工作并交还写锁。TUI 随后退出该会话；
Serve 取得写锁并加载会话后，桌面端才恢复写入。TUI 切换会话时，会交还旧会话
的镜像并注册新会话。

旧版 CLI 或未注册进程可能持有会话但没有同步连接。桌面端会说明这种状态，
不再提供无法生效的“取回”按钮。请在远端终端或窗口退出该会话，再从桌面端
重新打开。更新磁盘上的程序不会更新已经运行的 TUI 进程。
...
| 远程标签页进入旁观模式 | 远端终端或窗口持有会话。已连接同步时可取回；否则先在对应终端或窗口退出会话，再在这里重新打开 |
