...
for (const path of localeChunks) {
  ...
  // warning (~0.15 KiB gzip, +0.27% over the old 54.75 gate). The remote
  // credential work (disconnected-shell surface + host-form credential mode,
  // three locales) adds ~1 KiB gzip: zh-TW rides 56.62 KiB, zh 55.93 KiB
  // against the old 55.8/55.0 gates.
  const budget = name.startsWith("zh-TW-") ? 57.0 * 1024 : 56.5 * 1024;
