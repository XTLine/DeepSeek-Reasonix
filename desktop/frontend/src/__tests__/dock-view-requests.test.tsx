...
assert.equal(forwarded!.revealPathRequest?.path, first.path);
assert.equal(forwarded!.revealPathRequest?.acceptNavigation?.(), true);
assert(!("tabId" in forwarded!), "forwarding requests must not retain session data or callbacks");
await paint("project-a", "view-a");
assert.equal(forwarded!.revealPathRequest?.path, first.path);
assert.equal(forwarded!.revealPathRequest?.acceptNavigation?.(), false);
...
assert.equal(forwarded!.revealPathRequest?.path, next.path, "a new command while closed is delivered when the panel opens");
