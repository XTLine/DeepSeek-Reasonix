...
// The blank-session highlight contract: a tab sitting on a fresh unnamed
// session must expose the blank row identity (host\0workspace\0) so the tree
// highlights the pending row like a freshly created local topic, and once the
// serve lists the materialized session as Current the tab must adopt its
// Name/Path so the highlight migrates to the named row.
...
func TestRemoteTabAdoptsMaterializedSessionIdentity(t *testing.T) {
	...
	if tab.currentSessionPath != "/remote/sessions/fresh.jsonl" {
		t.Fatalf("adopted session path = %q, want /remote/sessions/fresh.jsonl", tab.currentSessionPath)
	}
	if tab.sessionReset {
...
func TestRemoteTabTopicIDSeparatorShape(t *testing.T) {
	...
	named := remoteTabTopicID(&remoteTab{ref: RemoteTabRef{HostID: "box", Workspace: "~/app"}, sessionName: "s1"})
